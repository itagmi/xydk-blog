...
export default async function AdminPage({
  ...
  const total = await prisma.post.count({ where: { category: "dev_note" } });
  const { totalPages, currentPage, skip, perPage } = getPaginationMeta(total, page);

  const posts = await prisma.post.findMany({
    where: { category: "dev_note" },
  ...
  return (
    ...
    >
          ...
          {posts.map((post) => (
            <li key={post.id} className="flex items-center justify-between gap-4 py-5">
              ...
              <div className="flex shrink-0 items-center gap-4">
                ...
                <ButtonLink href={`/admin/posts/${post.id}/edit`} variant="action">
                  Edit
                </ButtonLink>
      ...
      <Pagination basePath="/admin" currentPage={currentPage} totalPages={totalPages} />
    </AdminPageShell>
  );
}
